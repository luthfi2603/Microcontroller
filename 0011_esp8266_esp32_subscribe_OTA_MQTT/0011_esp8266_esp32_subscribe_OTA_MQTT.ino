...
constexpr char CURRENT_FIRMWARE_TITLE[] = "TEST-OTA";
...
constexpr char TOKEN[] = "qyKHxAEIxAv40mNM6Tr4";

// Thingsboard we want to establish a connection too
constexpr char THINGSBOARD_SERVER[] = "202.0.107.154";

// MQTT port used to communicate with the server, 1883 is the default unencrypted MQTT port,
// whereas 8883 would be the default encrypted SSL MQTT port
constexpr uint16_t THINGSBOARD_PORT = 1983U;
...
void progress_callback(const size_t & current, const size_t & total) {
  Serial.printf("Progress %.2f%%", static_cast<float>(current * 100U) / total);
  Serial.println();
}
...
void loop() {
  ...
  if (!tb.connected()) {
    ...
    Serial.printf("Connecting to: (%s) with token (%s)", THINGSBOARD_SERVER, TOKEN);
    Serial.println();
