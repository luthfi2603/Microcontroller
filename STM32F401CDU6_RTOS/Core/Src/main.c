...
int main(void)
{
  ...
  osThreadDef(senderTask, StartSenderTask, osPriorityIdle, 0, 128);
  senderTaskHandle = osThreadCreate(osThread(senderTask), NULL);

  /* definition and creation of receiverTask */
  osThreadDef(receiverTask, StartReceiverTask, osPriorityIdle, 0, 128);
...
void StartSenderTask(void const * argument)
{
  ...
  /* Infinite loop */
  for (;;) {
    xQueueSend(myQueueHandle, &counter, 0);
...
void StartReceiverTask(void const * argument)
{
  ...
  /* Infinite loop */
  for (;;) {
    if (xQueueReceive(myQueueHandle, &counter, portMAX_DELAY) == pdTRUE) {
